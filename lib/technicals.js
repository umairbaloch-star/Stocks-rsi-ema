...
import { resampleSeries } from "./rsi";
...
/**
 * EMA-based trend read (separate from the SMA-based `trend` field above,
 * which the existing confidence/breakout logic depends on and is left
 * untouched). Checks, in order: price > EMA20, EMA20 > EMA50, EMA50 >
 * EMA200 — each only "applicable" once its EMA has enough history (EMA200
 * needs 200 closes, out of reach for recently-listed or short-history
 * symbols). `label` is "Bullish" only when every applicable check passes,
 * "Bearish" only when every applicable check fails, "Neutral" otherwise —
 * matching the spec's "3/3 Bullish" framing while degrading gracefully
 * when there isn't 200 days of history to judge EMA200 alignment.
 */
export function calculateEmaTrend(closes) {
  const ema20 = calculateEMA(closes, 20).at(-1) ?? null;
  const ema50 = closes.length >= 50 ? calculateEMA(closes, 50).at(-1) : null;
  const ema200 = closes.length >= 200 ? calculateEMA(closes, 200).at(-1) : null;
  const price = closes.at(-1);

  const checks = [];
  if (ema20 !== null) checks.push(price > ema20);
  if (ema20 !== null && ema50 !== null) checks.push(ema20 > ema50);
  if (ema50 !== null && ema200 !== null) checks.push(ema50 > ema200);

  const total = checks.length;
  const passed = checks.filter(Boolean).length;
  const label = total === 0 ? null : passed === total ? "Bullish" : passed === 0 ? "Bearish" : "Neutral";
...
/**
 * MACD status derived from the histogram's current and previous values
 * (histogram = macdLine - signalLine, so its sign matches which side of the
 * signal line MACD is on, and a sign flip between the last two bars is
 * exactly a crossover on the most recent bar).
 */
function macdStatus(histogram) {
  const hist = histogram.at(-1);
  const prevHist = histogram.length >= 2 ? histogram.at(-2) : null;
  if (hist === null || hist === undefined) {
    return { status: "Neutral", freshCrossover: null };
  }
  const status = hist > 0 ? "Bullish Cross" : hist < 0 ? "Bearish Cross" : "Neutral";
  let freshCrossover = null;
  if (prevHist !== null && prevHist !== undefined) {
    if (prevHist <= 0 && hist > 0) freshCrossover = "bullish";
    else if (prevHist >= 0 && hist < 0) freshCrossover = "bearish";
  }
  return { status, freshCrossover };
}

/** Relative volume bucket, per the spec's example thresholds. */
function rvolBucket(rvol) {
  if (rvol === null) return null;
  if (rvol < 1.0) return "Low";
  if (rvol < 1.5) return "Normal";
  if (rvol < 2.0) return "High";
  return "Very High";
}

// A stock within this % of its support/resistance level is treated as
// "trading near" it for the nearSupport highlight — a round, documented
// heuristic, not derived from anything in the spec.
const NEAR_LEVEL_PCT = 3;

/**
 * Multi-timeframe EMA-trend confirmation across Daily/Weekly/Monthly
 * candles (resampled from the same adjusted daily series RSI already
 * resamples for its interval picker — see resampleSeries in lib/rsi.js).
 * Replaces the spec's 4H/1H legs, which would need an intraday feed PSX's
 * free EOD source doesn't provide. A timeframe with no applicable EMA
 * checks yet (e.g. too few weekly/monthly bars) is left out of both the
 * per-timeframe list and the alignment fraction's denominator, rather than
 * counted as neutral/bearish.
 */
export function calculateMultiTimeframeTrend(series) {
  const timeframes = [
    { key: "daily", label: "Daily", unit: "day", n: 1 },
    { key: "weekly", label: "Weekly", unit: "week", n: 1 },
    { key: "monthly", label: "Monthly", unit: "month", n: 1 },
  ];
  const reads = timeframes.map(({ key, label, unit, n }) => {
    const closes = resampleSeries(series, { unit, n }).map((p) => p.close);
    const trend = calculateEmaTrend(closes);
    return { key, label, trend: trend.label, score: trend.score };
  });
  const applicable = reads.filter((r) => r.trend !== null);
  const bullishCount = applicable.filter((r) => r.trend === "Bullish").length;
  return {
    timeframes: reads,
    alignment: applicable.length > 0 ? `${bullishCount}/${applicable.length}` : null,
  };
}
...
export function computeTradeAnalysis(series, rsi14, rsi5) {
  ...
  // --- Institutional-dashboard indicators (additive — nothing above this
  // block changes behavior for the existing entry/exit/confidence/factors
  // output already relied on elsewhere in the app). ---
  const emaTrend = calculateEmaTrend(closes);
  const { status: macdStatusLabel, freshCrossover } = macdStatus(histogram);
  const rvol = volumeRatio !== null ? Number(volumeRatio.toFixed(2)) : null;
  const distanceToSupportPct = Number((((price - support) / price) * 100).toFixed(2));
  const distanceToResistancePct = Number((((resistance - price) / price) * 100).toFixed(2));
  const multiTimeframe = calculateMultiTimeframeTrend(series);

  // Each applicable signal contributes -1 (bearish) / 0 (neutral) / +1
  // (bullish) to a consensus score. Confidence is how much the strongest
  // side dominates, not raw bullishness — a stock screaming "sell" is just
  // as high-confidence a read as one screaming "buy," it just means the
  // entry/exit numbers below aren't a good idea to act on right now.
  const signals = [];
  if (rsi14 !== null && rsi14 !== undefined) signals.push(rsi14 <= 35 ? 1 : rsi14 >= 65 ? -1 : 0);
  if (rsi5 !== null && rsi5 !== undefined) signals.push(rsi5 <= 20 ? 1 : rsi5 >= 80 ? -1 : 0);
  if (macdHist !== null) signals.push(macdHist > 0 ? 1 : macdHist < 0 ? -1 : 0);
  signals.push(trend === "up" ? 1 : trend === "down" ? -1 : 0);
  if (volumeRatio !== null) {
    // High volume amplifies whatever the trend already says; low/average
    // volume is a non-signal (0), not a bearish one.
    signals.push(volumeRatio >= 1.3 ? (trend === "down" ? -1 : trend === "up" ? 1 : 0) : 0);
  }
  signals.push(breakout === "breakout" ? 1 : breakout === "breakdown" ? -1 : 0);
  if (candle) signals.push(candle.bias === "bullish" ? 1 : -1);
  ...
  const agreement = signals.length ? dominant / signals.length : 0;
  const confidence = agreement >= 0.65 ? "High" : agreement >= 0.4 ? "Medium" : "Low";
  ...
  return {
    ...
    indicators: {
      emaTrend,
      macd: {
        line: macdLine.at(-1) !== null ? Number(macdLine.at(-1).toFixed(3)) : null,
        signal: signalLine.at(-1) !== null ? Number(signalLine.at(-1).toFixed(3)) : null,
        histogram: macdHist !== null ? Number(macdHist.toFixed(3)) : null,
        status: macdStatusLabel,
        freshCrossover,
      },
      rvol: {
        value: rvol,
        bucket: rvolBucket(rvol),
        spike: rvol !== null ? rvol > 1.5 : false,
      },
      support: {
        value: Number(support.toFixed(2)),
        resistance: Number(resistance.toFixed(2)),
        distanceToSupportPct,
        distanceToResistancePct,
        nearSupport: distanceToSupportPct <= NEAR_LEVEL_PCT,
      },
      multiTimeframe,
    },
  };
