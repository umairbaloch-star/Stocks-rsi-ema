...
// Predicted, MACD, EMA20/50, RSI14) into one call, so a user isn't left
// eyeballing 5+ columns that can legitimately disagree (e.g. a fresh MACD
// crossover firing while the last 5 sessions were still trending down — see
// computeNextDayPrediction above). Signal and Predicted get the heaviest
// weight because they're already themselves multi-factor reads (Signal =
// RSI14+EMA trend+MACD+volume; Predicted = realized momentum, trend-checked)
// — the raw single-indicator factors (MACD/EMA20/EMA50/RSI14) are included
// too, at lower weight, mainly to keep one bad/missing factor from silently
// dominating either of those two.
...
const STANCE_LABELS = {
  ...
  predicted: "Predicted next-session move",
  macd: "MACD State & Value",
...
/**
 * Combines `signal` (computeSimpleSignal), `prediction`
 * (computeNextDayPrediction), and the raw RSI14/EMA20/EMA50/MACD reads into
 * one -1..+1 weighted score and a single Strong Buy / Buy / Hold / Sell /
 * Strong Sell verdict. A factor with no data (e.g. EMA50 before 50 days of
 * history) is dropped and the remaining weights re-normalize, same pattern
 * as computeConfidenceLevel above. Predicted's continuous % move is capped
 * at ±2% so a single large projected swing can't outweigh every other vote.
 */
export function computeFinalStance({ signal, prediction, rsi14, ema20, ema50, price, macdScore }) {
  if (!signal) return null;

  const votes = {
    signal: signal.signal === "Buy" ? 1 : signal.signal === "Sell" ? -1 : 0,
    predicted:
      prediction && typeof prediction.predictedChangePercent === "number"
        ? Math.max(-1, Math.min(1, prediction.predictedChangePercent / 2))
        : null,
    macd: macdScore === null || macdScore === undefined ? null : macdScore / 40,
    ema20: ema20 === null || ema20 === undefined ? null : price > ema20 ? 1 : price < ema20 ? -1 : 0,
    ema50: ema50 === null || ema50 === undefined ? null : price > ema50 ? 1 : price < ema50 ? -1 : 0,
