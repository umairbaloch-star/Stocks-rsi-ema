 ...
 */
export function computeSimpleSignal(series, rsi14) {
  if (!series || series.length < MIN_HISTORY) return null;
  ...
  const { histogram } = calculateMACD(closes);
  const macdHist = histogram.at(-1);
  const macdStatus = macdHist === null ? null : macdHist > 0 ? "Bullish" : macdHist < 0 ? "Bearish" : "Neutral";
  ...
  if (macdStatus !== null) votes.push(macdStatus === "Bullish" ? 1 : macdStatus === "Bearish" ? -1 : 0);

  const net = votes.reduce((a, b) => a + b, 0);
  const thinVolume = volumeRatio !== null && volumeRatio < 0.7;

  let signal = "Watch";
  if (net >= 2 && !thinVolume) signal = "Buy";
  else if (net <= -2 && !thinVolume) signal = "Sell";
