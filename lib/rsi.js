...
export function adjustForCorporateActions(series) {
  ...
  // `open` (when present) is scaled by the same per-day factor as `close` so
  // the two stay on a consistent adjusted scale — needed for the open/close
  // candlestick-body checks in lib/technicals.js. `volume` is share count,
  // unaffected by a price adjustment, and is left alone.
  return series.map((p, i) => ({
    ...p,
    close: p.close * factors[i],
    ...(p.open != null ? { open: p.open * factors[i] } : {}),
  }));
}
...
/**
 * The mirror-image swing-exit signal, also on DAILY candles regardless of
 * view: RSI(14) ≥ 65 says the stock has run up into genuinely stretched
 * territory, RSI(2) ≥ 90 says the short-term rubber band is stretched right
 * now on the other side — together they flag a take-profit / avoid-fresh-
 * entry point, the mirror of BUY_SIGNAL's 35/10 (100 − 35 = 65, 100 − 10 = 90).
 */
export const SELL_SIGNAL = { rsi14Min: 65, rsi2Min: 90 };
