...
async function buildStockRecord(meta, forceInclude = false) {
  ...
  // core index, not a filtered-for-liquidity list), a symbol the user has
  // starred stays visible even if it's since gone thin (so a stock doesn't
  // vanish out from under a position they're already holding), and a
  // symbol the user is actively searching for (`forceInclude`, see
  // ensureSearchSymbol below) shows regardless of volume too — pasting a
  // known PSX symbol into the search box should always find it.
  const exempt = meta.isKse100 || state.watchedSymbols.has(meta.symbol) || forceInclude;
  ...
  adjusted[adjusted.length - 1] = {
    ...adjusted[adjusted.length - 1],
    close: price,
    // Same freshness reasoning as price: market-watch's today volume is
    // live, the eod feed's is settled/lagging — used by the volume-analysis
    // factor in lib/technicals.js.
    volume: typeof meta.volume === "number" ? meta.volume : adjusted[adjusted.length - 1].volume,
  };
  ...
  const record = {
    ...
    dayChangePercent: typeof meta.changePercent === "number" ? meta.changePercent : null,
    asOf: latest.date,
 ...
 *
 * Also computes the mirror-image sell-signal flag (see SELL_SIGNAL in
 * lib/rsi.js), a fixed take-profit/avoid-entry screener on daily candles,
 * and the full entry/exit/confidence trade analysis (see computeTradeAnalysis
 * in lib/technicals.js), which folds in MACD, moving averages, volume,
 * support/resistance, trend, and candlestick reads on top of RSI(5)/(14).
 */
function computeRsiFields(adjusted) {
  ...
  const sellSignal =
    r14 !== null && r2 !== null && r14 >= SELL_SIGNAL.rsi14Min && r2 >= SELL_SIGNAL.rsi2Min;
  const analysis = computeTradeAnalysis(adjusted, r14, r5);
  // Separate, much simpler RSI+EMA+MACD+volume → Buy/Sell/Watch read,
  // independent of the multi-factor `analysis` above (which still drives
  // Entry/Exit/Confidence) — kept as its own field rather than folded in.
  const signal = computeSimpleSignal(adjusted, r14, r5);
  const prediction = computeNextDayPrediction(adjusted);
  const finalStance = computeFinalStance({
    signal,
    prediction,
    rsi14: r14,
    ema20: signal?.ema20 ?? null,
    ema50: signal?.ema50 ?? null,
    price: adjusted[adjusted.length - 1]?.close ?? null,
    macdScore: signal?.macdScore ?? null,
  });
  return { rsi, buySignal, sellSignal, analysis, signal, prediction, finalStance };
}
...
async function loadPartition() {
  ...
  for (const s of symbols) {
    ...
    s.changePercent = info && typeof info.changePercent === "number" ? info.changePercent : null;
...
async function applyLivePrices() {
  ...
  for (const [symbol, record] of state.bySymbol) {
    ...
    const volumeChanged = info.volume != null && info.volume !== record.volume;
    if (info.volume != null) record.volume = info.volume;
    if (typeof info.changePercent === "number") record.dayChangePercent = info.changePercent;
    // Unchanged price AND volume — RSI grid and trade analysis still valid
    // (volume feeds the analysis's volume-ratio factor, so it alone can
    // still warrant a recompute even when price hasn't moved).
    if (record.price === info.current && !volumeChanged) continue;
    record.price = info.current;
    const updated = series.slice();
    updated[updated.length - 1] = {
      ...updated[updated.length - 1],
      close: info.current,
      volume: info.volume ?? updated[updated.length - 1].volume,
    };
...
/**
 * Lets a user paste ANY valid PSX symbol into the search box and have it
 * show up immediately — even if it's thinner than the volume floor, or
 * it's a "rest"-partition symbol the user hasn't fetched yet via "Load
 * more". Only fires on an exact (case-insensitive) symbol match against
 * the full equities list; a name/substring search that doesn't match a
 * real symbol is a no-op, so it never spends a wasted PSX request. Already-
 * loaded symbols are also a no-op — this only pays the one-time fetch cost
 * the first time a given symbol is searched.
 */
async function ensureSearchSymbol(searchSymbol) {
  if (!searchSymbol || !state.partitionReady) return;
  const symbol = searchSymbol.trim().toUpperCase();
  if (!symbol || state.bySymbol.has(symbol)) return;

  const meta =
    state.coreSymbols.find((s) => s.symbol === symbol) ||
    state.restSymbols.find((s) => s.symbol === symbol);
  if (!meta) return; // not a real PSX symbol — nothing to fetch

  try {
    const record = await buildStockRecord(meta, true);
    if (record) state.bySymbol.set(meta.symbol, record);
  } catch (err) {
    console.error(`[cache] search fetch failed for ${symbol}: ${err.name}: ${err.message}`);
  }
}

/**
 * `minVolumeOverride` lets the client tighten (never loosen) the volume
 * floor at request time — e.g. a user who only wants to see stocks trading
 * above 1M shares/day. Values at or below the server's MIN_DAILY_VOLUME are
 * a no-op: anything thinner than that was never fetched in the first place
 * (see buildStockRecord), so there's nothing left to filter further down.
 * KSE-100 constituents and the caller's watchlist stay exempt, same as the
 * base floor — the index and a tracked position should never disappear
 * just because the user tightened the filter. `searchSymbol` (the client's
 * current search box text) gets the same exemption — see ensureSearchSymbol
 * above — so pasting a symbol always finds it regardless of volume.
 */
export async function getStockData(includeRest = false, minVolumeOverride, searchSymbol) {
  ...
  await ensureSearchSymbol(searchSymbol);
  ...
  const effectiveMinVolume = Math.max(
    MIN_DAILY_VOLUME,
    Number.isFinite(minVolumeOverride) && minVolumeOverride > 0 ? minVolumeOverride : 0
  );
  const searchSymbolUpper = searchSymbol ? searchSymbol.trim().toUpperCase() : null;

  let extraHidden = 0;
  const stocks = Array.from(state.bySymbol.values()).filter((s) => {
    const exempt =
      s.isKse100 || state.watchedSymbols.has(s.symbol) || s.symbol === searchSymbolUpper;
    if (exempt || typeof s.volume !== "number" || s.volume >= effectiveMinVolume) return true;
    extraHidden++;
    return false;
  });
  ...
  return {
    ...
    baseMinVolume: MIN_DAILY_VOLUME,
