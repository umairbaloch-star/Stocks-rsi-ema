...
import { calculateCloseATR, lastValue } from "./indicators";
 ...
 *
 * Also computes `atrPct`/`volConfirmed` — the backtested BUY_SIGNAL
 * confirmation layer (see VOL_CONFIRM in lib/rsi.js) — on the raw DAILY
 * series, same reasoning as BUY_SIGNAL itself: never resampled to the
 * viewed period/interval.
 */
function computeRsiFields(adjusted) {
  ...
  const dailyCloses = adjusted.map((p) => p.close);
  const price = dailyCloses[dailyCloses.length - 1];
  const atr14 = lastValue(calculateCloseATR(dailyCloses, 14));
  const atrPct = atr14 !== null && price > 0 ? (atr14 / price) * 100 : null;
  const volConfirmed = atrPct !== null && atrPct >= VOL_CONFIRM.atrPctMin;
