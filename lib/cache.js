...
/**
 * Trend (50-day SMA), momentum (MACD histogram), volatility (close-to-close
 * ATR proxy) and volume-vs-average, plus the weighted composite score built
 * from them. Always computed on the raw daily series (never resampled) so
 * it means the same thing regardless of the period/interval currently
 * selected in the UI.
 */
function computeIndicatorFields(adjusted, { price, volume, r14, r2 }) {
  const closes = adjusted.map((p) => p.close);
  const sma50 = lastValue(calculateSMA(closes, 50));
