import { fetchSymbols, fetchEodSeries, fetchMarketWatch, fetchDividendHistory } from "./psx";
import {
  calculateRSISeries,
  lastRSIValue,
  resampleSeries,
...
import { computeTradeAnalysis } from "./technicals";
...
async function buildStockRecord(meta) {
  ...
  // symbol that's intentionally excluded (returns null below). Dividend
  // history is fetched alongside it (not after) purely to avoid adding
  // sequential latency; unlike the price history it's non-essential and
  // fetchDividendHistory already swallows its own errors into `[]`, so it
  // can never be the reason a symbol fails to load.
  const [series, dividendHistory] = await Promise.all([
    fetchEodSeries(meta.symbol),
    fetchDividendHistory(meta.symbol),
  ]);
  ...
  const record = {
    ...
    asOf: latest.date,
    dividends: summarizeDividends(dividendHistory),
...
function computeRsiFields(adjusted) {
  ...
  return { rsi, buySignal, sellSignal, analysis };
}
 ...
 * just because the user tightened the filter.
 */
export async function getStockData(includeRest = false, minVolumeOverride) {
  ...
  const stocks = Array.from(state.bySymbol.values()).filter((s) => {
    const exempt = s.isKse100 || state.watchedSymbols.has(s.symbol);
