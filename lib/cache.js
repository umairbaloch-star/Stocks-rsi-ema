...
async function buildStockRecord(meta) {
  ...
  const record = {
    ...
    asOf: latest.date,
    ...computeRsiFields(adjusted, { price, volume: meta.volume }),
  };
 ...
 *
 * Also computes the trend/volume/momentum confidence score (`lib/indicators.js`)
 * — deliberately on DAILY candles only, same as BUY_SIGNAL, never on the
 * viewed period/interval (see the fixed-screener note in AGENTS.md).
 */
function computeRsiFields(adjusted, { price, volume } = {}) {
  const rsi = {};
  ...
  const indicators = computeIndicatorFields(adjusted, { price, volume, r14, r2 });
  return { rsi, buySignal, ...indicators };
}

/**
 * Trend (200-day SMA — see docs/PROGRESS.md item 14 for why 200 and not
 * 50), momentum (MACD histogram), volatility (close-to-close ATR proxy) and
 * volume-vs-average, plus the weighted composite score built from them.
 * Always computed on the raw daily series (never resampled) so it means the
 * same thing regardless of the period/interval currently selected in the UI.
 */
function computeIndicatorFields(adjusted, { price, volume, r14, r2 }) {
  const closes = adjusted.map((p) => p.close);
  const sma200 = lastValue(calculateSMA(closes, 200));
  const macd = calculateMACD(closes);
  const macdHistogram = lastValue(macd.histogram);
  const prevMacdHistogram = lastValue(macd.histogram.slice(0, -1));
  const atr14 = lastValue(calculateCloseATR(closes, 14));

  // Average volume excludes the still-forming last bar (today) — comparing
  // today's live volume against a baseline that already includes today
  // would understate how unusual today's volume is.
  const priorVolumes = adjusted
    .slice(0, -1)
    .map((p) => p.volume)
    .filter((v) => Number.isFinite(v) && v > 0);
  const last20 = priorVolumes.slice(-20);
  const avgVolume20 = last20.length > 0 ? last20.reduce((a, b) => a + b, 0) / last20.length : null;

  const currentVolume = Number.isFinite(volume) ? volume : null;
  ...
  return {
    ...
    macdHistogram: macdHistogram !== null ? Number(macdHistogram.toFixed(3)) : null,
    ...
    scoreBreakdown: composite ? composite.breakdown : null,
  };
}
...
async function applyLivePrices() {
  ...
  for (const [symbol, record] of state.bySymbol) {
    ...
    Object.assign(record, computeRsiFields(updated, { price: info.current, volume: record.volume }));
  }
